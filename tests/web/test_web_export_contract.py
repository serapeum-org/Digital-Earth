"""Round-2 review M9/M10 — where ``WebMap.save_animation``'s warning lands, and what ``duration=`` may convert
to.

Two defects in the one place the package converts a deprecated parameter rather than renaming it:

* **M9** — the animation entry point forwarded ``**kwargs`` to a second method, which resolved ``duration=``
  itself. The shared helper's hand-counted ``stacklevel`` then landed the warning about the caller's keyword
  inside ``export.py``, a file the caller does not own. The assertion here is on the warning's **filename**,
  since a misattributed warning still satisfies ``pytest.warns``.
...
class TestTheKeywordWarningLandsOnTheCallersLine:
    """M9 — a warning attributed to ``export.py`` names a line the caller cannot edit."""

    def test_duration_is_attributed_to_the_caller(self, two_step_map):
        """``WebMap.save_animation(duration=...)`` must point at the user's file, not at ``export.py``.

        Args:
            two_step_map: The stubbed map, output path and encoder record.

        Test scenario:
            The forwarding method added a frame the shared helper's ``stacklevel`` never counted, so the
            deprecation notice about the caller's own keyword pointed inside the package. ``duration=`` is
            resolved in ``save_animation`` itself now, one frame below the caller.
        """
        scene, out, _ = two_step_map
        with warnings.catch_warnings(record=True) as caught:
            warnings.simplefilter("always")
            scene.save_animation(str(out), duration=0.5)
        by_message = {
            str(record.message).split(":")[0]: record
            for record in caught
            if issubclass(record.category, DeprecationWarning)
        }
        assert len(by_message) == 1, [str(record.message) for record in caught]
        for label, record in by_message.items():
            assert record.filename == HERE, f"{label} landed in {record.filename}"

    def test_the_keyword_still_converts_the_rate_it_was_given(self, two_step_map):
        """Moving the resolution up a frame must not change what the old call renders.
        ...
        with warnings.catch_warnings():
            ...
            assert scene.save_animation(str(out), duration=0.5) == out
        assert recorded["duration"] == pytest.approx(0.5), recorded

    def test_no_rate_at_all_uses_the_shared_default(self, two_step_map):
        """A call naming no rate animates at the package default.
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            scene.save_animation(str(out))
        assert recorded["duration"] == pytest.approx(1.0 / DEFAULT_FPS), recorded

    def test_both_spellings_at_once_is_still_refused(self, two_step_map):
        """``save_animation(fps=..., duration=...)`` names one rate twice, so it is a ``TypeError``.
        ...
        with warnings.catch_warnings():
            ...
            with pytest.raises(TypeError, match="pass only fps="):
                scene.save_animation(destination, fps=4.0, duration=0.5)
    ...
    def test_a_non_finite_duration_is_refused(self, bad, two_step_map):
        ...
        with warnings.catch_warnings():
            ...
            with pytest.raises(ValueError) as excinfo:
                scene.save_animation(destination, duration=bad)

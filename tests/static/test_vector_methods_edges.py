...
def value_points():
    ...
    return _fc(
        gpd.GeoDataFrame({"v": [1.0, 2.0, 3.0, 4.0]}, geometry=pts, crs="EPSG:32618")
    )
...
def far_side_points():
    ...
    pts = [
        ...
        Point(180, 0),
        Point(150, -20),
    ]  # 180/150 lon = far side
    return _fc(
        gpd.GeoDataFrame({"v": [1.0, 2.0, 3.0, 4.0]}, geometry=pts, crs="EPSG:4326")
    )
...
class TestQuadtreeAggregation:
    ...
    def test_named_agg(self, value_points, agg, expected):
        ...
        assert float(arr[0]) == pytest.approx(expected), (
            f"{agg} -> {arr[0]}, expected {expected}"
        )
...
class TestQuadtreeCells:
    ...
    def test_nmin_drops_sparse_cells(self):
        ...
        kept = Map._quadtree_cells(
            xs, ys, agg_fn=lambda idx: float(len(idx)), nmax=2, nmin=2
        )
        assert sorted(c[4] for c in kept) == [4.0], (
            f"unexpected kept counts: {[c[4] for c in kept]}"
        )
...
class TestDefensiveBranches:
    ...
    def test_quadtree_cells_no_progress_below_nmin_dropped(self):
        ...
        ys = np.zeros(3)
        kept = Map._quadtree_cells(
            xs, ys, agg_fn=lambda idx: float(len(idx)), nmax=1, nmin=5
        )
        assert kept == [], (
            f"expected no cells (3 < nmin 5 at the no-progress stop), got {kept}"
        )
    ...
    def test_api_wrappers_swallow_colorbar_errors(self, monkeypatch, value_points):
        ...
        assert isinstance(
            api.voronoi(value_points, column="v", crs=value_points.epsg), MapCls
        )
        assert isinstance(
            api.cartogram(polys, scale="v", column="v", crs=polys.epsg), MapCls
        )
        assert isinstance(
            api.quadtree(value_points, column="v", nmax=1, crs=value_points.epsg),
            MapCls,
        )
        assert isinstance(api.kde(value_points, crs=value_points.epsg), MapCls)
        assert isinstance(
            api.sankey(lines, column="w", scale="w", crs=lines.epsg), MapCls
        )
...
class TestVectorHelpers:
    ...
    def test_clip_geometry_gdf_unioned(self):
        ...
        gdf = gpd.GeoDataFrame(
            geometry=[box(0, 0, 1, 1), box(2, 2, 3, 3)], crs="EPSG:32618"
        )
    ...
    @pytest.mark.parametrize(
        "limits, expected_first, expected_last", [((0.2, 1.0), 0.2, 1.0)]
    )
